...
function Moonridge(name, connectPromise, isDefault) {
  ...
  var connDfd = Promise.defer();
  self.connectPromise = connDfd.promise;
  Promise.resolve(connectPromise).then(function(rParams) {
    self.rpc = RPC(rParams.url, rParams.hs);
    self.socket = self.rpc.io;
    if (rParams.auth) {
...
//                    TODO call getModel for all models
  ...
  self.getModel = function(name) {
    ...
    if (!model) {
      ...
      var toExpose = {MR:{}};
      toExpose.MR[name] = model.clientRPCMethods;
      self.rpc.expose(toExpose);
    }
